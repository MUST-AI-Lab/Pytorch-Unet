    ...
    with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{args.epochs}', unit='img') as pbar:
        ...
        for batch in train_loader:
            ...
            if args.accumulation_step==1:
                optimizer.zero_grad()
                loss.backward()
                #normlazation  final lyer
                weight_norms,grad_norms = weight_norm(net,args) #collect weight norm for final layer
                v_weight_norms = torch.from_numpy(weight_norms).type(torch.FloatTensor).to(device=device, dtype=torch.float32).view((-1,1,1,1))
                v_grad_norms = torch.from_numpy(grad_norms).type(torch.FloatTensor).to(device=device, dtype=torch.float32).view((-1,1,1,1))
                net.final.weight.data = net.final.weight.data / (v_weight_norms+1e-7)
                net.final.weight.grad = net.final.weight.grad / (v_grad_norms+1e-7)
                #----------------------------
                weight_norms,grad_norms = weight_norm(net,args) #collect weight norm for final layer
                for i in range(args.num_classes):
                    avg_meters['final_norm_{}'.format(i)].update(weight_norms[i])
                    avg_meters['loss_gd_norm_{}'.format(i)].update(grad_norms[i])
                #----------------------------
