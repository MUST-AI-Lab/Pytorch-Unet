...
if __name__ == '__main__':
    ...
    datamaker.get_pairs("total.txt")
    datamaker.get_disturibution()
    n_val = len(datamaker)
    #force batch size = 1
    val_loader = DataLoader(datamaker, batch_size=1, shuffle=False, num_workers=args.num_workers, pin_memory=True,collate_fn=default_collate_with_weight)
        ...
        for batch  in val_loader:
            ...
            if net.n_classes > 1:
                ...
                for i in range(imgs.shape[0]):
                    ...
                    if True:
                        _,_,id_str =  datamaker.pairs[batch_count]# keep batch size is 1 and id maps to batch count 
                        datamaker.showrevert_cp2file_origin(img,s_true_mask,s_pred,args.experiment,'{}'.format(id_str))
                        show = True
