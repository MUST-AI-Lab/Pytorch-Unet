...
def statistic_prediction(predict,gt):
    # get soft max
    t_predict = torch.from_numpy(predict).type(torch.FloatTensor)
    t_gt = torch.from_numpy(gt).type(torch.LongTensor)
    t_softmax = torch.softmax(t_predict, dim=0)  # 按列SoftMax,列和为1
    # get onehot
    shp_preds = t_predict.shape
    ...
    # to numpy for next step
    softmax = t_softmax.cpu().numpy()
    onehot = t_onehot.cpu().numpy()

    # to slide each category
    avg_slide=[]
    avg_gt_slide=[]
    avg_ngt_slide=[]
    val_slide=[]
    ...
    val_ngt_slide=[]
    for i in range(32):
        avg_slide.append(np.mean(softmax[i]))
        mean = np.mean(softmax[i])
        val_slide.append(np.mean((softmax[i]-mean)*(softmax[i]-mean)))
        onehot_slide = onehot[:,:,i]
        n_onehot_slide = 1- onehot_slide
        gt_softmax = softmax[i] * onehot_slide
        ngt_softmax = softmax[i] * n_onehot_slide

        if np.sum(onehot_slide)>0:
            avg_gt_slide.append(np.sum(gt_softmax)/np.sum(onehot_slide))
            mean = np.sum(gt_softmax)/np.sum(onehot_slide)
            val_gt_slide.append(np.sum((gt_softmax-mean)*(gt_softmax-mean)* onehot_slide)/np.sum(onehot_slide))
        else:
            avg_gt_slide.append(0)
            val_gt_slide.append(0)

        if np.sum(n_onehot_slide)>0:
            avg_ngt_slide.append(np.sum(ngt_softmax)/np.sum(n_onehot_slide))
            mean = np.sum(ngt_softmax)/np.sum(n_onehot_slide)
            val_ngt_slide.append(np.sum((ngt_softmax-mean)*(ngt_softmax-mean)* n_onehot_slide)/np.sum(n_onehot_slide))
        else:
            avg_ngt_slide.append(0)
            val_ngt_slide.append(0)


    return avg_slide,avg_gt_slide,avg_ngt_slide,val_slide,val_gt_slide,val_ngt_slide
...
if __name__ == '__main__':
    ...
    for i in range(args.num_classes):
        ...
        log['iou_{}'.format(i)] = []
        log['avg_slide_{}'.format(i)] = []
        log['avg_gt_slide_{}'.format(i)] = []
        log['avg_ngt_slide_{}'.format(i)] = []
        log['val_slide_{}'.format(i)] = []
        log['val_gt_slide_{}'.format(i)] = []
        log['val_ngt_slide_{}'.format(i)] = []
        ...
        for batch  in val_loader:
            ...
            if net.n_classes > 1:
                ...
                for i in range(imgs.shape[0]):
                    ...
                    #----------------------------for check prediction probability
                    avg_slide,avg_gt_slide,avg_ngt_slide,val_slide,val_gt_slide,val_ngt_slide=statistic_prediction(s_pred,s_true_mask)
                    for clazz in range(args.num_classes):
                        log['avg_slide_{}'.format(clazz)].append(avg_slide[clazz])
                        log['avg_gt_slide_{}'.format(clazz)].append(avg_gt_slide[clazz])
                        log['avg_ngt_slide_{}'.format(clazz)].append(avg_ngt_slide[clazz])
                        log['val_slide_{}'.format(clazz)].append(val_slide[clazz])
                        log['val_gt_slide_{}'.format(clazz)].append(val_gt_slide[clazz])
                        log['val_ngt_slide_{}'.format(clazz)].append(val_ngt_slide[clazz])
                    #----------------------------------------------------------------------
            ...
            batch_count+=1
        pbar.close()
        pd.DataFrame(log).to_csv('./result/{}_apd.csv'.format(args.experiment),index=None)
