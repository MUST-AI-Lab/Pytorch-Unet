    ...
    with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{args.epochs}', unit='img') as pbar:
        ...
        for batch in train_loader:
            ...
            if 'weight' in batch:
                ...
                #N=np.array(range(32))+2
                N=np.array([20,10,200,20,150,150,10,100,50,60,10,10,10,10,150,10,10,20,20,20,20,300,10,10,100,100,160,100,100,100,500,100]
)+2
                beta = (N-1)/N
                effective_num = 1.0 - np.power(beta, samples_per_cls)
                weights = (1.0 - beta) / effective_num
                weights *=  ( np.squeeze(count.numpy(),axis=0)>0)
                #区分 weights and weight
                weight = distribution2tensor(args.num_classes,weights,true_masks.numpy())
                weight = torch.tensor(weight).float()
                if ajust_loss >0:
                    weight = weight * ajust_loss
